...
plot(keel~days,data=keels.plot,axes=F,ylab="Keel Prominece",xlab="Days in Captivity",main="Keel Regression in Captive Octopuses",type="n")
box(lwd=2)
axis(1)
axis(2)
...
# Redoing the data analysis taking into account the octopuses put into condos
...
keels.nocondo=keels.df[keels.df$type!="condo",]
...
```{r}
keels.withcondo.lme=lme(keel~days*burrow+type,
             random=~1|octo,
             correlation=corAR1(form=~days|octo),
              data=keels.df)
keels.withcondo.anova=Anova(keels.withcondo.lme,type="III")
keels.withcondo.anova
